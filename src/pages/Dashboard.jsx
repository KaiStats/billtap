...
export default function Dashboard() {
  ...
  useEffect(() => {
    base44.entities.Receipt.list("-created_date", 50).then((data) => {
      setReceipts(data);
      setLoading(false);
    });
  }, []);

  const totalOwed = receipts.reduce((sum, r) => {
    return sum + (r.participants || []).filter(p => p.payment_status !== "paid").reduce((s, p) => s + (p.amount_owed || 0), 0);
  }, 0);
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            { label: "Bills Split", value: receipts.length, icon: Receipt, color: "text-purple-600 bg-purple-50" },
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {receipts.map((receipt) => {
              ...
              return (
                <Link key={receipt.id} to={createPageUrl(`ReceiptDetail?id=${receipt.id}`)}>
                  <Card className="rounded-2xl border-0 shadow-sm hover:shadow-md transition-shadow cursor-pointer">
                    <CardContent className="p-5 flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <div className="font-bold text-gray-900">{receipt.title}</div>
                          <div className="text-sm text-gray-500 flex items-center gap-2 mt-0.5">
                            <Users className="w-3.5 h-3.5" />
                            {total} people · ${(receipt.total_amount || 0).toFixed(2)}
                          </div>
                        </div>
                      </div>
                      <div className="flex items-center gap-3">
                        <div className="text-sm text-gray-500">{paid}/{total} paid</div>
                        <Badge className={statusColors[receipt.status] || statusColors.pending}>
