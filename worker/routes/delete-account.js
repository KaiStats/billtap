...
async function cancelSubscription(env, subscriptionId) {
  if (!subscriptionId || !env?.STRIPE_SECRET_KEY) return false;
  try {
    const res = await fetchWithTimeout(
    ...
    if (!res.ok) {
      console.error(`delete-account: could not cancel ${subscriptionId}: ${res.status}`);
      return false;
    }
    return true;
  } catch (err) {
    console.error('delete-account: cancelling the subscription threw', err?.message);
    return false;
  }
}
...
export async function onRequestPost({ request, env, ctx, requestId = null }) {
  ...
  try {
    ...
    const profile = (await svc.entity('Profile')
      .filter({ id: user.id }, { select: 'id,stripe_subscription_id' }))[0] || null;
    const cancelled = await cancelSubscription(env, profile?.stripe_subscription_id);
    ...
    await audit(env, ctx, {
      ...
      request,
      requestId,
      detail: { subject_fp: await fingerprint(user.id), cancelled_subscription: cancelled, ...removed },
    });
